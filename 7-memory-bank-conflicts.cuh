...
__global__ void gemm_memory_bank_conflicts_swizzle(int M, int K, int N, float *A, float *B, float *C) {
...
template <const int BM, const int BK, const int BN, const int TM, const int TN>
__global__ void gemm_memory_bank_conflicts_extra_col(int M, int K, int N, float *A, float *B, float *C) {
  const uint cRow = blockIdx.y;
  const uint cCol = blockIdx.x;
